...
class RoteiroController {
    ...
    def indexMain(Integer max) {
        params.max = Math.min(max ?: 10, 100)
        respond Roteiro.list(params), model:[roteiroInstanceCount: Roteiro.count()]
    }

    def list(){
        def listRot = Roteiro.list();
        def countListRot = listRot.size();

        return render(template:"list", model:[roteiro:listRot, countListRot: countListRot], flash: flash)
    }
    ...
    def editStatus(Roteiro roteiroInstance) {
        respond roteiroInstance
    }
