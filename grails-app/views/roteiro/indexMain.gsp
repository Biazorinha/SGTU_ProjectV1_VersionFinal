...
<header class="navbar navbar-expand navbar-dark flex-column flex-md-row bd-navbar">
	<div class="navbar-nav-scroll">
		<ul class="navbar-nav bd-navbar-nav flex-row">
			<li class="nav-item">
				<a class="nav-link " href="${createLink(uri: '/')}" onclick="ga('send', 'event', 'Navbar', 'Community links', 'Bootstrap');">Acessar</a>
			</li>
			%{--<li class="nav-item">--}%
			%{--<a class="nav-link" href="${createLink(uri:'/')}" onclick="ga('send', 'event', 'Navbar', 'Community links', 'Bootstrap');">Cadastar Roteiro</a>--}%
			%{--</li>--}%
		</ul>
	</div>
</header>
		...
		<table class="table table-sm">
			...
			<g:each in="${roteiroInstanceList}" status="i" var="roteiroInstance">
				<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
					<th scope="row">1</th>
					<td>${fieldValue(bean: roteiroInstance, field: "numRoteiro")}</td>
